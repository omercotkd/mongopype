...
from mongopype.stages.geo_near import verify_geo_near, GeoNearSpec


_VALID_SPEC: GeoNearSpec = {
...
def test_geo_near_first_with_near_and_distance_field_valid():
    valid, errors = verify_geo_near(
        _VALID_SPEC, V_60, pipeline_index=0, pipeline_length=1, is_atlas=False
    )
...
def test_geo_near_not_first_fails():
    valid, errors = verify_geo_near(
        _VALID_SPEC, V_60, pipeline_index=1, pipeline_length=2, is_atlas=False
    )
...
def test_geo_near_missing_distance_field_below_81_fails():
    spec: GeoNearSpec = {"near": {"type": "Point", "coordinates": [0.0, 0.0]}}
    valid, errors = verify_geo_near(
...
def test_geo_near_missing_distance_field_at_81_ok():
    spec: GeoNearSpec = {"near": {"type": "Point", "coordinates": [0.0, 0.0]}}
    valid, _ = verify_geo_near(
        spec, V_81, pipeline_index=0, pipeline_length=1, is_atlas=False
    )
    assert valid is True
